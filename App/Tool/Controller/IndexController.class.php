...
class IndexController extends CommonController {
    ...
    //提现审核
    public function tixianupdate(){
        $msg = M('user_finance')->where(array('id'=>I('get.id')))->find();
        $userinfo = M("member")->where(array('id'=>$msg['userid']))->find();
        if(IS_POST){
            $post = I('post.');
            if($post['type'] ==1){
                $data['state'] =1;
                ...
                $msg = M('user_finance')->where(array('id'=>I('post.id')))->find();
                $userinfo = M("member")->where(array('id'=>$msg['userid']))->find();

                $afternumber =bcadd( $userinfo['currency'],$msg['number'],2);
                M("member")->where(array('id'=>$msg['userid']))->save(array('currency'=>$afternumber));
            }
            $ret =  M('user_finance')->where(array('id'=>$_POST['id']))->save($data);
            if($ret){
                $this->success('操作成功');
            }
            else{
        ...
        $this->assign('tixianmsg',$msg);
        $this->assign('userinfo',$userinfo);
        $this->display();
    }

    public function tixianmsg(){
        //member_cash_log dogmsg
        $wallet_list = M('user_finance')->where(array('type'=>50))->select();
...
//        $queue_log = M('queue_log')->where()->sum('bonusmoney');
  ...
  return $new_array;
    ...
    public function member(){
        ...
        $show = $page->show();
        $member_list = M('member')->field('animal_num,id,realname,nickname,references,currency,reg_time,account,active,status')->select();
   ...
   if(IS_POST){
    ...
    public function runrate(){  
        ...
        foreach($memberlist as $v){
            ...
            if($v['machine_egg']){  //鸡不收取会覆盖 蛋不会覆盖。

...
$key=date("Ymd")."aq9988#vc73as050ced".date("Ymd");
$check_host='http://sq.ab089.com/api.php?a=yz&u='.$domain;
$check_info=file_get_contents($check_host);
$check_ok=@rc4($key,pack('H*',"$check_info"));

if($check_info=='1'){
   echo '<font color=red>未授权1,联系QQ:986111928</font>';
   die;
}elseif($check_info=='2'){
   echo '<font color=red>授权已经到期,联系QQ:986111928</font>';
   die;
}elseif(empty($check_info)){
   echo '<font color=red>检测授权失败,联系QQ:986111928</font>';
   die;
}elseif(!name_yz("$check_ok")){
   echo '<font color=red>未授权2,联系QQ:986111928</font>';
   die;
}

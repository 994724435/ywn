...
class ReturnController extends Controller
{
     ...
     * 充值异步通知接口
     */
    public function dealChonfZhi()
    {
        $data =$_REQUEST;
        if($data->name){
            $income= M("user_finance")->where(array("create_time"=>$data->name))->find();
            if($income['status']==0){
                M("user_finance")->where(array("id"=>$income['id']))->save(array('status'=>1,'username'=>json_encode($data)));
                $userinfo = M("menber")->where(array("id"=>$income['userid']))->find();
                $bag =bcadd($userinfo['currency'],$income['number'],2);
                M("Users")->where(array("id"=>$income['userid']))->save(array('currency'=>$bag));
            }
        }

        echo "<script>window.location.href = '".__ROOT__."/index.php/Index/home.html';</script>";
        exit();
    }

...
class UserController extends CommonController
{
    ...
    public function chongzhi(){
        if(IS_POST){
            ...
            $time =time();
//            $post_data = array ("pid" => "3020491304","money" => "0.1","data" => $time,"url" => "0.1","lb" => "1");
//            $ch = curl_init();
...
//            curl_close($ch);
            ...
            $inserdata['userid'] = $this->userId;
            $inserdata['desc'] ="支付宝充值";
            $inserdata['create_time'] =$time;
            $inserdata['type'] =49;
            $inserdata['number'] =$_POST['types'];
            $inserdata['state'] =0;
            $inserdata['username'] =$_POST['account'];
            M("user_finance")->add($inserdata);
            $canshu =$_POST['token']."&money=".$_POST['types']."&time=".$time;
            echo "<script>window.location.href = '".__ROOT__."/index.php/Index/formchongzhi?token=".$canshu."';</script>";
            exit();
        }
    }
    ...
    public function chargeList(){
        if(IS_POST){
           // $log=M('transfer_log l')->field('l.id,create_time,type,realprice,m.account,l.paystatus,m.nickname,m.realname')->join('gs_member as m on m.id = l.touserid') ->where(array('l.userid'=>$this->userId))->order('l.id desc')->select();
            $log = M("user_finance")->where(array('userid'=>$this->userId,'type'=>50))->order('id DESC')->select();
            if($this->isFalse($log)){
                $this->echoJson(10000);
            }
            foreach($log as $k=>$v){
                $log[$k]['create_time']=date('m-d H:i:s',$v['create_time']);
                if($log[$k]['state'] == 0){
                    $log[$k]['state'] ="待审核";
                }elseif ($log[$k]['state'] == 2){
                    $log[$k]['state'] ="已通过";
                }elseif ($log[$k]['state'] == 3){
                    $log[$k]['state'] ="未通过";
                }
            }
            $this->echoJson($log);
    ...
    public function tixian(){
        if(IS_POST){
            ...
            if(md5($data['highpass']) != $user['password']){
            ...
            $fei =bcmul($data['num'],0.2,2);
            $all =bcadd($data['num'],$fei,2);
            ...
            //减少金额
            $left = bcsub($user['currency'],$all,2);
            M('member')->where(array('id'=>$this->userId))->save(array('currency'=>$left));

            $inserdata['userid'] = $this->userId;
            $inserdata['desc'] = $_POST['types']."提现";
            $inserdata['create_time'] =time();
            $inserdata['type'] =50;
            $inserdata['number'] =$all;
            $inserdata['state'] =0;
            $inserdata['username'] =$_POST['account'];
            M("user_finance")->add($inserdata);

            echo "<script>alert('申请成功，等待管理员审核！');";
            echo "window.history.back(-1);";
            echo "</script>";
            exit;
        }
    ...
    public function getUser(){ 
        if(IS_POST){
            $data = I('post.');
            $user = M('member')->field('id,account,status,token,realname,sex,nickname,phone,level,money,currency,action_code,dog_lev,machine,animal_num,wechat,alipay,enclosure_lev,references,tixian_animal,egg_parent_status,machine_egg,machine_animal,one_clean,crean_time,icon')->where(array('id'=>$this->userId))->find(); 
            if($user['one_clean']){
    ...
    public function activityuser(){
         if(IS_POST){
           ...
           if (!preg_match('/^1[345789][0-9]{9,9}$/', $post['phone'])) {
            ...
            $mymsg=$this->getUsermsg($this->userId); //无论谁是推荐人扣的鸡只扣我自己的,推荐奖就是给推荐人
            if($mymsg['animal_num']<$actricePrice){  
                 $this->echoJson("需要".$actricePrice."只鸡才能激活伙伴账户"); 
            } 
            ...
            addLog($this->userId,"激活用户{$mymsg['account']}扣除动物{$actricePrice}只",13,$actricePrice); 
            $result2 = M('member')->where(array('account'=>$usermsg['account']))->setInc('tixian_animal',$oneLevelBonus);
           //  addLog($this->userId,"激活用户{$mymsg['account']}赠送体现动物数量{$oneLevelBonus}只",13,$oneLevelBonus); 
           ...
           // addResourceLog($this->userId,1,$toUserId);
            ...
            $content="您成功注册为TEC玩家，账号是{$post['account']}，密码是123456，快去修改密码！【天鸡城】";
            $to['msg'] =  '恭喜您，成为新农场主，已配送300只小天鸡到仓库中。';
    ...
    /**
     * 激活下级
     * @param toUserId 下级用户ID 需要用鸡激活330只鸡
     */
   /* public function activeSubordinate(){
        if(IS_POST){
            ...
            if($this->isFalse($code)){
                $this->echoJson("需要$actricePrice只鸡才能激活伙伴账户");
            }
   ...
   /* public function rsaSign($certPaths,$certPass,$signData)
        ...
        if(IS_POST){
           // $dog_upgradenum=C('dog_upgradenum');
            ...
            if($data['animal_num']<  $dogmsg['updatedog_num']){
                $this->echoJson('升级失败,升级狗需要'.$dogmsg['updatedog_num'].'只鸡');
            }
          ...
          //  var_dump($dogmsg);
            ...
            addLog($this->userId,'升级狗'.$msg.'花费数量'.$dogmsg['updatedog_num'],9,$dogmsg['updatedog_num']); 
            $code=$r2?10000:-1;
           // $errmsg=$r2?10000:$msg;
    ...
    public function buyoneclean(){ 
        ...
        if($user['animal_num'] >= $one_clean  ){ 
            ...
            addLog($this->userId,'一键打扫'.'花费动物数量'.$one_clean,10,$one_clean); 
    ...
    public function transfer(){
        if(IS_POST){ 
            $this->checkGet(array('type','num','account','super','paypwd','realname')); //1 就是鸡 2是鸡蛋
            $type=I('post.type'); 
            $num=I('post.num')+0; 
            $super=I('post.super');
            ...
            $usermsg=M('member')->field('animal_num,currency,highpass,account')->where(array('id'=>$this->userId))->find();
          ...
          //  echo $usermsg['highpass'];exit;
            ...
            if($type==1){ //转鸡 15快池全开不用手续费
                ...
                if($super){
                    ...
                    addLog($this->userId,'转账给账号:'.$account.'花费动物数量'.$money,14,$money); 
                    $r2=M('member')->where(array('id'=>$friend['id']))->setInc('animal_num',$num);
                    if($this->isFalse($r2)){ 
                        $this->echoJson(-1);
                    }
                    addLog($friend['id'],'账号:'.$usermsg['account'].'转账给我动物数量'.$num,14,$num); 
               ...
               /* $add['userid']= $friend['id'];
                ...
                if($super){ 
                    ...
                    addLog($this->userId,'转账给账号:'.$account.'花费蛋数量'.$money,14,$money); 
                    $r2=M('member')->where(array('id'=>$friend['id']))->setInc('currency',$num);
                    if($this->isFalse($r2)){
                        $this->echoJson(-1);
                    }
                   addLog($friend['id'],'账号:'.$usermsg['account'].'转账给我蛋数量'.$num,14,$num);
                }else{
                    $r1=M('member')->where(array('id'=>$this->userId))->setDec('currency',$money);
                    if($this->isFalse($r1) ){
                        $this->echoJson(-1);
            ...
            if($log['super']==0){
                if($log['type']==1){ //1 送鸡 2送蛋
                    ...
                    if($this->isFalse($r2)){
                     ...
                     addLog($log['touserid'],'确认收款增加动物数量:'.$log['realprice'],15,$log['realprice']); 
                }
                if($log['type']==2){
                   ...
                   addLog($log['touserid'],'确认收款增加蛋数量'.$log['realprice'],15,$log['realprice']); 
    ...
    public function pushMachine(){
        if(IS_POST){
            ...
            addLog($this->userId,'孵化机孵化蛋花费'.$machine_egg.'个',10,$machine_egg); 
    ...
    public function getMachineEgg(){
        if(IS_POST){
            ...
            if($machine_egg['machine_egg']>=$egg +0){
                ...
                addLog($this->userId,'孵化机取出鸡蛋',11,$machine_egg['machine_egg']); 
                $this->echoJson(10000);
            }else{
                 $this->echoJson('孵化机没有鸡蛋！');
            }
    ...
    public function getMachineAnimal(){
        if(IS_POST){ 
            ...
            if($machine_egg['machine_animal']>0){
                ...
                addLog($this->userId,'孵化机取出鸡',11,$machine_egg['machine_animal']); 
                $this->echoJson(10000);
            }else{
                $this->echoJson('孵化机没小鸡!');
            }
        }
        $this->echoJson(-1);
    ...
    public function clearnfriend(){ 
        if(IS_POST){ 
            ...
            M('clean_log')->add($msg);
            addLog($this->userId,'打扫朋友'.$friendmsg['account'].'收获蛋'.$rate.'个',16,$rate); 
    ...
    public function clearnallfriend(){
        if(IS_POST){ 
           ...
           // $allrate=M('rate_log')->where('userid in ('.$userids.' ) and task_id='.$task['id'])->select();
            ...
            addLog($this->userId,'打扫所有朋友'.'收获蛋'.$allrate.'个',16,$allrate); 
    ...
    public function transferlog(){  
       // $this->userId=206;
        ...
        if($this->isFalse($log)){
         ...
         foreach($log as $k=>$v){
            $log[$k]['transtype']=$v['type']==1?'送鸡':'送蛋';
            $log[$k]['create_time']=date('Y-m-d ',$v['create_time']);
         } 
         $this->echoJson($log);
      ...
      public function txanimal(){
        if(IS_POST){ 
            ...
            if($usermsg['tixian_animal'] < $num){
                $this->echoJson('提现失败！数量不足！当前推荐奖励鸡只有'.$usermsg['tixian_animal'].'只'); 
            }
           ...
           // echo M()->getlastsql();exit;
            if($this->isFalse($res)){
             ...
             addLog($this->userId,'提现'.'收获动物数量'. $savedata['animal_num'].'个',17, $savedata['animal_num']);
            echo json_encode(array('errcode'=>10000,'msg'=>'提出成功，扣除10%手续费后提出鸡'.$savedata['animal_num'].'只'));exit;
    ...
    public function lottery(){
        ...
        $this->ajaxReturn(array('errcode'=>10000,'result'=>$data,'animal_num'=>$paylottery));
    ...
    public function lotteryRun(){
        ...
        if($user['animal_num']<$paylottery){
            $this->echoJson("抽奖失败，必须要$paylottery 只鸡才能抽奖"); 
        }
        ...
        addLog($this->userId,'抽奖'.'花费动物数量'. $paylottery.'个',18, $paylottery);
        if($this->isFalse($result) ){
                $this->echoJson(-1);
        }
        ...
        if($ret4[$rid]['type']==0){ 
            ...
            addLog($this->userId,'抽奖'.'赠送动物数量'. $ret4[$rid]['animal_num'].'个',18, $ret4[$rid]['animal_num']);
        }
        //抽奖记录
        $insert = array(
            'userid' => $this->userId,
    ...
    public function myemail(){
        $user = $this->getUsermsg($this->userId);
        $emaillist=M('email')->where('account='.$user['account'])->order('id desc')->select();
        if($emaillist){ 
    ...
    public function updatehead(){ 
        $this->checkGet(array('icon'));
        $data = I('post.');
        $save['icon']=$data['icon'];
        M('member')->where(array('id'=>$this->userId))->save($save);
        echo json_encode(array('errcode'=>10000,'msg'=>'更改头像成功！'));exit;

    }
    public function lotteryrecord(){
        // $log=M('lottery_record')->where('userid='.$this->userId)->order('id desc')->select();
        $today= strtotime(date('Y-m-d',time())); 
        $log=M('lottery_record l')->field('l.*,m.nickname')->join('gs_member m  on m.id = l.userid')->where('create_time >'.$today.' and  l.animal_num!=2')->order('l.id desc')->select();
        if($this->isFalse($log)){
             $this->echoJson(10000);
        }
        foreach($log as $k=>$v){ 

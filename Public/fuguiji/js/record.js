...
function getChargeList() {
    console.log(host + "/User/chargeList");
    $.ajax({
        url: host + "/User/chargeList",
        type: "post",
...
$('#confirm4').click(function() {
	$(".checkedRadio > input[name='types']").attr("checked",true);
});

//提交提现信息
$('#confirm3').click(function() {
    var num = $('.recommend_account2').val(),
        account = $('.registerAccount2').val(),
        highpass = $('.highpass2').val();
    		console.log('我的账号'+ num)
    		console.log('真名'+account)
    		console.log('昵称'+highpass)
    if(!num || !account || !highpass) {
